...
import io.qzz.nekobounce.utils.extras.ColorManager
import io.qzz.nekobounce.utils.extras.MovementUtils
...
object TargetStrafe : Module("TargetStrafe", Category.MOVEMENT) {
    ...
    private fun canStrafe(): Boolean {
        ...
        return state && targetEntity != null && (!holdSpaceValue.get() || mc.gameSettings.keyBindJump.isKeyDown) && (!onlySpeedValue.get() || NekoBounce.moduleManager[Speed::class.java]?.state == true) && (!onlyFlightValue.get() || NekoBounce.moduleManager.getModule(
            "Fly"
        )!!.state)
    }
