...
import io.qzz.nekobounce.utils.block.BlockUtils.getBlock
...
object MovementUtils : MinecraftInstance {
    ...
    fun FlyBasic(speed: Float) {
        ...
        io.qzz.nekobounce.utils.movement.MovementUtils.strafe(speed)
    }
    ...
    fun strafe() {
        io.qzz.nekobounce.utils.movement.MovementUtils.strafe(getSpeed())
    }
    ...
    fun strafe(speed: Float) {
        if (!io.qzz.nekobounce.utils.movement.MovementUtils.isMoving()) return
        mc.thePlayer.motionX = -sin(io.qzz.nekobounce.utils.movement.MovementUtils.direction) * speed
        mc.thePlayer.motionZ = cos(io.qzz.nekobounce.utils.movement.MovementUtils.direction) * speed
    }
    fun strafe(event: MoveEvent, speed: Float) {
        if (!io.qzz.nekobounce.utils.movement.MovementUtils.isMoving()) return
       event.x = -sin(io.qzz.nekobounce.utils.movement.MovementUtils.direction) * speed
       event.z = cos(io.qzz.nekobounce.utils.movement.MovementUtils.direction) * speed
    }
    ...
    fun doTargetStrafe(
        ...
        strafe_ *= _direction
        var strafeYaw = io.qzz.nekobounce.utils.rotation.RotationUtils.getRotationsEntity(curTarget).yaw.toDouble()
    ...
    fun move(speed: Float) {
        if (!io.qzz.nekobounce.utils.movement.MovementUtils.isMoving()) return
        val yaw = io.qzz.nekobounce.utils.movement.MovementUtils.direction
    ...
    val movingYaw: Float
        get() = (io.qzz.nekobounce.utils.movement.MovementUtils.direction * 180f / Math.PI).toFloat()
